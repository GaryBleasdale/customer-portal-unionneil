...
export default async function renameBoletos(
	pdfBuffers: { buffer: Buffer; originalName: string }[],
			...
			pdfParser.on("pdfParser_dataReady", (pdfData) => {
				try {
					...
					if (text.includes("RECIBO DO PAGADOR")) {
						const pagadorMatch = text.match(/PAGADOR: (.*?)(?=\s*cnpj|\n|$)/i);
						if (pagadorMatch && pagadorMatch[1]) {
							...
							console.log("pagadorName", pagadorName);
							resolve({
								buffer,
								filename: `${pagadorName}_boleto_unionneil.pdf`,
							});
						} else {
