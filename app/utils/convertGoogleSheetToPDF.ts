...
export default async function convertGoogleSheetToPDF(
	...
	async function listFiles(auth) {
		const res = await drive.files.list({
			auth,
			q: `'${sourceFolderId}' in parents and (mimeType='application/vnd.google-apps.spreadsheet' or mimeType='application/vnd.google-apps.document')`,
			fields: "files(id, name)",
		});
		console.log("fatura files in the source folder", res.data.files);
	...
	async function processSheets() {
		...
		for (const file of files) {
			...
			console.log(
				`Downloaded ${file.name}.pdf - Size: ${(pdfBuffer.length / 1024 / 1024).toFixed(2)}MB`,
			);
			console.log(
				`Memory usage after download: ${JSON.stringify({
